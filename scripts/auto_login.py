...
class AutoLogin:
    ...
    def save_cookie(self, value):
        ...
        # === 核心修改：动态获取 Secret 名称 ===
        secret_key_name = os.environ.get('SESSION_SECRET_KEY', 'GH_SESSION')
        
        # 自动更新 Secret
        if self.secret.update(secret_key_name, value):
            self.log(f"已自动更新 {secret_key_name}", "SUCCESS")
            self.tg.send(f"🔑 <b>Cookie 已自动更新</b>\n\n{secret_key_name} 已保存")
        else:
            # 通过 Telegram 发送
            self.tg.send(f"""🔑 <b>新 Cookie</b>

请更新 Secret <b>{secret_key_name}</b>:
<code>{value}</code>""")
